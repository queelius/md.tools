...
We allow converting between three representations:
lists of integer vectors, Boolean vectors, and lists of charsets (sets
represented with strings, e.g., "{ 1, 2 }".
Thus, the inverse of `md_boolean_matrix_to_list` is just
`md_list_to_boolean_matrix` and so on.
...
print(meta)
```
Note that we removed the `row.names` attribute, since it's quite long and
uninformative.
